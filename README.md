...
### 可选：watermark-remover 图片后处理

> [!NOTE]
> 用于对接口返回的 `data:image/...;base64,...` 结果做二次修正（例如去水印场景）。
> 该功能默认关闭，开启后如果外部命令执行失败会自动回退原图，不影响主流程响应。

1. 安装 `watermark-remover`（推荐放在独立 Python 虚拟环境）：
...
backend:
  postprocess:
    watermarkRemover:
      enabled: true
      command: "/绝对路径/到/.venv/bin/watermark-remover"
      method: "opencv"
```

3. 重启服务后生效（`/admin/restart`）。
